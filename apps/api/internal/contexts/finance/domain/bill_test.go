...
func TestNewRecurringBill_Valid(t *testing.T) {
	b, err := NewRecurringBill("b-1", "user@test.com", "Netflix", "Subscription", 49900, FrequencyMonthly, 15, time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), nil, false, nil)
...
func TestNewRecurringBill_WithEndDate(t *testing.T) {
	...
	b, err := NewRecurringBill("b-2", "user@test.com", "Rent", "Housing", 1500000, FrequencyMonthly, 1, time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), &end, true, strPtr("RENT"))
	require.NoError(t, err)
...
func TestNewRecurringBill_Weekly(t *testing.T) {
	b, err := NewRecurringBill("b-3", "user@test.com", "Grocery", "Food", 500000, FrequencyWeekly, 0, time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), nil, false, nil)
	require.NoError(t, err)
...
func TestNewRecurringBill_EmptyName(t *testing.T) {
	_, err := NewRecurringBill("b-4", "user@test.com", "  ", "Cat", 100, FrequencyMonthly, 1, time.Now(), nil, false, nil)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "name is required")
}

func TestNewRecurringBill_EmptyCategory(t *testing.T) {
	_, err := NewRecurringBill("b-5", "user@test.com", "Test", "", 100, FrequencyMonthly, 1, time.Now(), nil, false, nil)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "category is required")
}

func TestNewRecurringBill_ZeroAmount(t *testing.T) {
	_, err := NewRecurringBill("b-6", "user@test.com", "Test", "Cat", 0, FrequencyMonthly, 1, time.Now(), nil, false, nil)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "amount must be positive")
}

func TestNewRecurringBill_InvalidFrequency(t *testing.T) {
	_, err := NewRecurringBill("b-7", "user@test.com", "Test", "Cat", 100, Frequency("biweekly"), 1, time.Now(), nil, false, nil)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "invalid frequency")
}

func TestNewRecurringBill_DayOfMonthOutOfRange(t *testing.T) {
	_, err := NewRecurringBill("b-8", "user@test.com", "Test", "Cat", 100, FrequencyMonthly, 32, time.Now(), nil, false, nil)
	assert.Error(t, err)
...
func TestNewRecurringBill_EndDateBeforeStart(t *testing.T) {
	...
	_, err := NewRecurringBill("b-9", "user@test.com", "Test", "Cat", 100, FrequencyMonthly, 1, time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), &end, false, nil)
	assert.Error(t, err)
...
func TestNewRecurringBill_NameTooLong(t *testing.T) {
	...
	_, err := NewRecurringBill("b-10", "user@test.com", string(long), "Cat", 100, FrequencyMonthly, 1, time.Now(), nil, false, nil)
	assert.Error(t, err)
