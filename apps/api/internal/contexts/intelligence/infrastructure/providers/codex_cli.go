...
func (p *codexCLI) Complete(ctx context.Context, req ChatRequest) (*ChatResult, error) {
	...
	// Ephemeral + isolated: no session files, no ambient tools.
	args = append(args,
		"--no-rollout-persistence",
		"--no-strict-tool-policy",
		"--no-mcp",
		"--no-hooks",
		"--no-memory",
		"--search", "disabled",
	...
	case err := <-done:
		if err != nil {
			return nil, fmt.Errorf("codex exec failed: %v: %s", err, truncate(stderr.String(), 300))
		}
