...
func TestRequireBearerToken(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "remote", header: "Bearer test-token", remote: "192.0.2.1:1234", status: http.StatusUnauthorized},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			RequireBearerToken("test-token", true)(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
