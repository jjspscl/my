...
export async function getMagicLinkUrl(email: string): Promise<string> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    const resp = await fetch(`${MAILPIT_API}/messages`)
    ...
    if (msg) {
      const detailResp = await fetch(`${MAILPIT_API}/message/${msg.ID}`)
...
export async function clearMailpitMessages(): Promise<void> {
  const resp = await fetch(`${MAILPIT_API}/messages`, { method: 'DELETE' })
